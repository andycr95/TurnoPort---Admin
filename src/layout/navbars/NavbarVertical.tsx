...
const NavbarVertical = (props: NavbarVerticalProps) => {
  const location = usePathname();
  const CustomToggle = ({ children, eventKey, icon }: CustomToggleProps) => {
  ...
  const generateLink = (item: MenuProps) => {
    return (
      <Link
        href={item.link!}
        className={`nav-link ${
          location === item.link ? "bg-primary text-white active" : ""
        }`}
        onClick={(e) =>
  ...
  return (
    <Fragment>
      <SimpleBar style={{ maxHeight: "100vh" }}>
        ...
        >
          {DashboardMenu.map(function (menu: MenuProps, index: number) {
            if (menu.grouptitle) {
              return (
